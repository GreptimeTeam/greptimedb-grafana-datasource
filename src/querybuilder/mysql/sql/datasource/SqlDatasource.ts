...
import { greptimeTypeToGrafana } from '../../../../greptimedb';
...
export abstract class SqlDatasource extends DataSourceWithBackend<SQLQuery, SQLOptions> {
  ...
  private transformSqlResult(result, query) {
    const frames = result.data.output.map((frame) => {
      const data = {values: Array.from({length: frame.records.rows[0]?.length || 0}).map((v, index) => {
        return frame.records.rows.map((data) => data[index])
      })}
      const schema = {
        fields: frame.records.schema.column_schemas.map((schemaItem) => {
          return {
            name: schemaItem.name,
            type: greptimeTypeToGrafana[schemaItem.data_type],
            typeInfo: {
              frame: greptimeTypeToGrafana[schemaItem.data_type],
              nullable: true
            }
          }
        }),
        meta: {
          executeQueryString: query.rawSql
        },
        refId: query.refId
      }
      return {
    ...
    return lastValueFrom(
      getBackendSrv()
        .fetch<BackendDataSourceResponse>({
          url: `api/datasources/proxy/uid/${this.uid}/v1/sql`,
          method: 'POST',
          headers: {...this.getRequestHeaders(), 'Content-Type': 'application/x-www-form-urlencoded'},
          data: {
            // from: range.from.valueOf().toString(),
            // to: range.to.valueOf().toString(),
            // queries,
            sql: queries[0].rawSql
          },
          requestId: refId,
        })
        .pipe(
          map((res: FetchResponse<BackendDataSourceResponse>) => {
            let rsp = {data: []}
            if (res.data.output[0]?.records.rows.length) {
              const result = this.transformSqlResult(res, queries[0])
              rsp = toDataQueryResponse({data: result}, queries);
            }
